defmodule Pinchflat.Podcasts.PodcastExportTest do
  ...
  describe "export_source/2" do
    ...
    test "excludes media that lives outside the podcast library", %{source: source} do
      # Downloaded before the source became a podcast: still under the media
      # library, so its enclosure URL would be unreachable on the static server
      outside = Path.join(Application.get_env(:pinchflat, :media_directory), "old-episode.mp3")
      FilesystemUtils.cp_p!(media_filepath_fixture(), outside)
      outside_item = media_item_fixture(%{source_id: source.id, media_filepath: outside})

      inside = downloaded_episode(source, "New [id].mp3")

      assert :ok = PodcastExport.export_source(source, @url_base)

      feed = File.read!(feed_path(source))
      assert String.contains?(feed, "#{inside.uuid}")
      refute String.contains?(feed, "#{outside_item.uuid}")
    end
  end

  describe "feed_generated?/1" do
    test "is false before an export has run", %{source: source} do
      refute PodcastExport.feed_generated?(source)
    end

    test "is true once feed.xml has been written", %{source: source} do
      downloaded_episode(source, "Title [id].mp3")
      assert :ok = PodcastExport.export_source(source, @url_base)

      assert PodcastExport.feed_generated?(source)
    end

    test "is false for a source without a slug" do
      refute PodcastExport.feed_generated?(%Pinchflat.Sources.Source{slug: nil})
    end
  end
