...
- Dashboard/source tables in hidden tabs load lazily: the `LazyTab` JS hook (`assets/js/hooks.js`) pushes a one-time `lazy_load` event when a table's tab first becomes visible (tab switches update `location.hash`), so `MediaItemTableLive` and `HistoryTableLive` defer their queries and PubSub subscriptions until then — a hidden "Pending" history table never re-runs its expensive query on `job:state` broadcasts. The hook element must keep its id + `phx-hook` across all render states and re-fire via the hook's `reconnected()` callback: LiveView only mounts hooks on newly added elements, so a reconnect (which remounts the server un-loaded) would otherwise strand the table on "Loading...". The home page's default-visible "Downloaded" tab passes `"lazy" => false` and loads eagerly instead (content in the initial HTML) since deferring a visible table only delays first paint. Media-table reload broadcasts are scoped per source (`media_table:<id>`). The `/live` socket enables the `longpoll` transport and the client sets `longPollFallbackMs` so browsers that can't open the websocket (Safari refuses `ws://` to `.local` mDNS hostnames) fall back to HTTP long-polling instead of running the page dead.
