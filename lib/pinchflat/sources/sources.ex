defmodule Pinchflat.Sources do
  ...
  @doc """
  Returns the relevant output path template for a source. A source override wins;
  otherwise podcast sources use a flat, slug-rooted layout (so the podcast library
  can be served in place) and everything else uses the media profile's template.
